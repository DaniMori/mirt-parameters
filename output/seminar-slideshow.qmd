...
AXIS_LAB_POS <-   .5       # Replace default axis label position for slideshow
LINE_WIDTH   <-   .75      # Replace default line width for slideshow
AXIS_COLOR   <- "darkgray" # Replace default grid line color for slideshow
```
...
theme_set( # `ggplot` output configuration
  theme_classic(
    ...
    theme(
      ...
      panel.grid.minor = element_line(linetype = "17", color = "grey70")
    )
)
```
...
simple_item_loadings_wide |>
  ...
  theme(
    ...
    axis.title.y       = element_text(vjust = axis_pos),
...
simple_item_loadings_rot_plot +
  geom_abline(
    data    = rotated_axes,
    mapping = aes(slope = slope, intercept = 0, color = factor)
  )
```
:::
...
$$
I_1 = \tau_1 + \lambda_{11} F_1^p + \lambda_{21} F_2^p + ... + \lambda_{n1} F_n^p + U_1^p \\
\vdots                                                                            \\
I_k = \tau_k + \lambda_{1k} F_1^p + \lambda_{2k} F_2^p + ... + \lambda_{nk} F_n^p + U_k^p
$$
...
::: fragment
$$
I_1 = \tau_1 + \mathbf{\lambda}_{1}^T \mathbf{F} + U_1
$$
:::
...
El producto del vector de pesos por el de puntuaciones factoriales es lo que se llama un "producto vectorial", que consiste simplemente en multiplicar dos a dos los términos de cada vector y sumar todos.
:::
...
Su respuesta se calcularía así: (hacer los cálculos).
...
::::: {.column width="40%"}
$$
  i_k^* = \tau_k + \mathbf{\lambda}_k^T \mathbf{F} + U_k
$$
...
$$
  P(I_k = 1) = \int_{-\infty}^0 \mathcal{N}(i_k^*) \delta i_k^* \\
             = \Phi_N \left(\frac{\tau_k + \mathbf{\lambda}_k^T \mathbf{F}}{\sqrt{var(U_k)}}\right)
$$
:::
:::::
...
latent_response |>
  ...
  scale_y_continuous(
    ...
    minor_breaks = NULL,
    labels       = NULL,
    limits       = range(ERROR_POINTS) * 1.12,
    expand       = expansion()
  )           +
  ...
  theme(
    ...
    panel.grid.major = element_line(linetype = "17")
  )                              +
  coord_flip()
```
:::
...
logit_curve_data <- tibble(
  ...
  rename_with(stringr::str_remove, pattern = '\\.{3}')
...
$$
  P(I_k = 1) = \Phi_L (\mathbf{a}_k^T \mathbf{\theta} + d_k)
$$
...
plot_ly(
  ...
  type = 'surface',
  hovertemplate = 'P(I<sub>k</sub> = 1) = %{z:.2f}<extra></extra>',
  showscale = FALSE,
  hoverinfo = 'Z'
) |>
  layout(
    scene = list(
      ...
      zaxis = list(title = "P(I<sub>k</sub> = 1)"),
      camera = list(
        center = list(x = -.1, z = -.2),
...
contour_plot <- mirt_item_probs |>
  ...
  geom_contour2(
    ...
    label.placer = label_placer_fraction(frac = .18, rot_adjuster = 0)
  ) +
  scale_x_continuous(
    minor_breaks = NULL,
...
Aquí aparece otra vez la discriminación pero ahora en forma vectorial, que se multiplica por el vector de rasgo latente de la persona, y un parámetro de intersección, como antes (pero no puede haber un parámetro de dificultad simplemente, o uno por cada dimensión, porque serían indistinguibles entre sí).
...
$$
  \begin{align}
    P(I_k = 1) &= \Phi_L \left([1, 0.5]
      ...
      &= \frac{1}{1 + e^{-0}} = \frac{1}{1 + 1} = \frac{1}{2} = 0.5
  \end{align}
$$
:::
:::
...
$$
  \begin{align}
    P(I_k = 1) &= \Phi_N \left(
      \frac{\mathbf{\lambda}_k^T \mathbf{F} + \tau_k}{\sqrt{var(U_k)}}
    \right) \\
    P(I_k = 1) &= \Phi_L (\mathbf{a}_k^T \mathbf{\theta} + d_k)
  \end{align}
$$
...
$$
  \begin{align}
    \mathbf{a}_k &= \frac{1.7}{\sqrt{var(U_k)}} \mathbf{\lambda}_k \\
    d_k &= \frac{1.7}{\sqrt{var(U_k)}} \tau_k
  \end{align}
$$
:::
...
$$
MID = \frac{-d_k}{\sqrt{\mathbf{a}_k^T\mathbf{a}_k}}
$$
...
::: notes
Por ese motivo propone un parámetro de "dificultad" (lo llama, para nosotros sería de posición) que sea de algún modo equivalente a la dificultad en el modelo unidimensional.
...
mirt_item_spikes <- mirt_item_coords |>
  select(item, trait_1 = origin_transf_1, trait_2 = origin_transf_2) |>
...
contour_plot +
  ...
  geom_point(
    ...
    mapping = aes(origin_transf_1, origin_transf_2),
    inherit.aes = FALSE,
    size = I(4)
  ) +
  scale_x_continuous(
