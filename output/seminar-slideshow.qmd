...
```{r graphical-output-conf, cache=FALSE}
```
...
simple_item_loadings <- tribble(
  ...
  'I3',  'F1',    ITEM_BASE_LOADING + 2 * ITEM_DIST,
  'I4',  'F2',    ITEM_BASE_LOADING,
...
item3_spikes <- item3_spikes                         |> 
  ...
  mutate(n = (row_number() + 1) %% 2 |> as.factor()) |>
  pivot_wider(names_from = type, values_from = F1:F2)

spike_colors = c(
  '1' = unname(PALETTE_BLUE['light']),
  '0' = unname(PALETTE_RASPBERRY['light'])
)
...
method_item_loadings <- tribble(
  ...
  'I6',  'M1',    -ITEM_BASE_LOADING + 2 * ITEM_DIST
)
