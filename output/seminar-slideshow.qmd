...
VECTOR_WIDTH <- 2.5        # Replace default vector width for slideshow
```
...
$$
  P(Y_i = 1) = \int_{-\infty}^0 \mathcal{N}(y_i^*) \delta y_i^* \\
             = \Phi_N \left(\frac{\tau_i + \mathbf{\lambda}_i^T \mathbf{F}}{\sqrt{var(U_i)}}\right)
$$
:::
:::::
...
$$
  MID = \frac{-d_i}{\sqrt{\mathbf{a}_i^T\mathbf{a}_i}}
$$
...
Por lo tanto, la "dificultad (posición) multidimensional" viene dada por dos elementos, un número real que representa cuánto se desplaza la recta de inflexión del ítem respecto del origen y en qué sentido, y un vector de cosenos que determina la dirección en la que se desplaza.
...
mirt_item_coords |>
  ...
  ggplot(
    aes(
      0, 0,
      xend = end_transf_1 - origin_transf_1,
      yend = end_transf_2 - origin_transf_2,
  ...
  scale_x_continuous(
    limits       = c(-1, 2), ## TODO: Change limits
    breaks       = -1:2,
    minor_breaks = NULL,
    name         = NULL,
    oob          = oob_keep
