...
latent_response_plot <- latent_response |>
  ggplot(aes(x = response, y = factor(point), fill = ..x.. > 0))           +
  ...
  labs(x = "i~k~^*^", y = NULL)                                            +
  scale_y_discrete(labels = ERROR_POINTS)                                  +
  scale_x_continuous(breaks = 0)                                           +
  scale_fill_manual(
    values = c(`FALSE` = "white", `TRUE` = unname(PALETTE_BLUE["white"]))) +
  theme(
    legend.position = "none",
    axis.title.y = element_markdown(angle = 90),
    panel.grid.major.y = element_line(linetype = "17")
  )                                                                        +
  coord_flip()
...
probit_plot <- probit_curve_data |>
  ...
  labs(x = "i~k~^*^", y = "P(I~k = 1)")                               +
  scale_y_continuous(breaks = 0:1, minor_breaks = NULL, limits = 0:1) +
  scale_x_continuous(minor_breaks = ERROR_POINTS, breaks = -1:1 * 3)  +
  theme(
    axis.title   = element_markdown(),
    axis.title.y = element_markdown(angle = 90),
  )

## TODO: Correct error break points
