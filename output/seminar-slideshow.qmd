...
Estas nuevas coordenadas (transformadas), que ahora están en un espacio ortogonal, queremos que representen una distribución en la que las correlaciones sean nulas, es decir, que la matriz de correlaciones transformada ($R^*$ la vamos a llamar) sea una matriz identidad (unos en la diagonal, ceros en el resto).
...
h_grid_limits <- trunc(x_limits / vec_2[2]) + c(-1, 1)
h_grid_values <- h_grid_limits[1]:h_grid_limits[2]
h_grid_interc <- h_grid_values * vec_2[2]
h_grid_labpos <- h_grid_values + vec_1_tan * x_limits[1]

v_grid_limits <- trunc(y_limits / vec_1[1]) + c(-1, 1)
v_grid_values <- v_grid_limits[1]:v_grid_limits[2]
v_grid_xcoord <- v_grid_values * vec_1[1]
v_grid_interc <- - v_grid_xcoord * vec_2_tan
v_grid_labpos <- v_grid_values # TODO

ggplot() +
  geom_abline(intercept = h_grid_interc, slope = vec_1_tan) +
  geom_abline(intercept = v_grid_interc, slope = vec_2_tan) +
  scale_x_continuous(
    minor_breaks = NULL,
    breaks = v_grid_labpos,
    labels = v_grid_values,
    limits = x_limits,
    expand = expansion(),
    name   = NULL,
    sec.axis = dup_axis(name = "*&theta;*~2~", labels = NULL)
  ...
  scale_y_continuous(
    minor_breaks = NULL,
    breaks = h_grid_labpos,
    labels = h_grid_values,
    limits = y_limits,
    expand = expansion(),
    name   = NULL,
    sec.axis = dup_axis(name = "*&theta;*~1~", labels = NULL)
  ...
  theme(
    axis.ticks       = element_blank(),
    axis.line        = element_blank(),
    axis.title       = element_markdown(valign = vec_2_tan * y_limits[2]), # TODO
    axis.title.y     = element_markdown(halign = vec_1_tan * x_limits[2])
  )
```
:::
::: column
