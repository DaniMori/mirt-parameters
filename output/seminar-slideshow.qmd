...
F1_COLOR <- PALETTE_BLUE["normal"]
F2_COLOR <- PALETTE_RASPBERRY["normal"]

item_colors   <- c(F1_COLOR |> rep(3), F2_COLOR |> rep(3))
factor_colors <- c('F1' = unname(F1_COLOR), 'F2' = unname(F2_COLOR))
...
latent_response_plot <- latent_response |>
  ggplot(aes(x = response, y = point, fill = ..x.. > 0, group = point)) +
...
## TODO: Correct error break points and fig height
latent_response_plot / probit_plot
```
:::
::::::
...
nLab <- c("I1", "I2", "I3", "I4", "I5", "I6", "F1", "F2") |> setNames(nm = _)
Bidir <- FALSE |> rep(6)
Directed <- TRUE |> rep(6)
Shape <- c("square", "square", "square", "square", "square", "square", 
"circle", "circle")
...
qgraph::qgraph(Edgelist, 
      ...
      barColor = tColor, barLength = thresholdSize, barsAtSide = ThreshAtSide, 
