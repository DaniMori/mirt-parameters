...
# Define grid limits as a function of the axes limits:
...
vec_1_tan <- vec_1[2] / vec_1[1]
vec_2_tan <- vec_2[2] / vec_2[1]
vec_1_mod <- vec_1^2 |> sum() |> sqrt()
vec_2_mod <- vec_2^2 |> sum() |> sqrt()

linear_transf <- function(points, basis, x = x, y = y) {
  
  any_group_cols <- points |> select(-({{x}}:{{y}})) |> ncol()
  
  if (!any_group_cols) points <- points |> rownames_to_column()
  
  points |>
    pivot_longer({{x}}:{{y}}) |>
    group_by(across(-(name:value))) |>
...
grid_bounds <- bind_rows(
  h = grid_box |>
    filter(xor(vec_1_tan > 0, x_lim == y_lim)) |>
    mutate(limit = y_lim, slope = vec_1_tan),
  v = grid_box |>
    filter(xor(vec_2_tan > 0, x_lim == y_lim)) |>
    mutate(limit = x_lim, slope = vec_2_tan),
  .id = "grid"
) |>
  # linear_transf(basis) |>
  mutate(intercept = y - slope * x) # Compute their intercept
...
base_intercepts <- tibble(
  h = vec_2[2] - vec_1_tan * vec_2[1],
  v = vec_1[2] - vec_2_tan * vec_1[1]
) |>
  pivot_longer(everything(), names_to = "grid", values_to = "intercept")

grid_specs <- grid_bounds |>
  select(grid, limit:intercept) |>
  left_join(base_intercepts, by = "grid", suffix = c("_limit", "_base")) |>
  mutate(
    n_limit = trunc(intercept_limit / intercept_base) %/%
      break_step *
      break_step
  )
  
grid <- grid_specs |>
  select(-intercept_limit) |>
  pivot_wider(names_from = limit, values_from = n_limit) |>
  group_by(grid) |>
  reframe(
    slope,
    coord = seq(from = inf, to = sup, by = break_step),
    intercept = coord * intercept_base
  ) |>
  mutate(
    label_pos = if_else(
      grid == "h",
      intercept + x_limits[1] * vec_1_tan,
      intercept + y_limits[1] / vec_2_tan
    ),
    ref = as.numeric(!intercept)
  )

# h_grid <- tibble(
#   pos = seq(from = h_grid_limits[1], by = break_step, to = h_grid_limits[2]),
#   x   = 0L,
#   y   = pos,
#   ref = as.numeric(!y)
# ) |>
...
#     mutate(
#       slope     = vec_1_tan,
#       intercept = y - vec_1_tan * x 
#     )
# 
# 
# h_grid_interc <- h_grid_values * vec_2[2]
# h_grid_labpos <- h_grid_interc + vec_1_tan * x_limits[1]
# h_linetype    <- if_else(h_grid_interc == 0, 'solid', '13')
# 
# v_grid_limits <- (trunc(y_limits / abs(vec_1[1]) + x_limits / abs(vec_2_tan)) + c(-1, 1)) %/% break_step * break_step
# v_grid_values <- seq(from = v_grid_limits[1], by = break_step, to = v_grid_limits[2])
# v_grid_xcoord <- v_grid_values * vec_1[1]
# v_grid_labpos <- v_grid_xcoord # Initial position of the x labels
scale_linetype <- c(`1` = 'solid', `0` = 'dashed')

if (is.infinite(vec_2_tan)) {
  
  v_grid <- grid_bounds |>
    filter(grid == "v") |>
    mutate(
      unit    = vec_1[1],
      x_coord = trunc(x / unit) %/% break_step * break_step
    ) |>
    select(grid, limit, x_coord, unit) |>
    pivot_wider(names_from = "limit", values_from = x_coord) |>
    reframe(
      x_coord = seq(from = inf, to = sup, by = break_step) * unit
    ) |>
    mutate(ref = as.numeric(!x_coord))
...
# else {
#   
#   v_grid_labpos <- v_grid_labpos + y_limits[1] / vec_2_tan
#   v_grid_interc <- -v_grid_xcoord * vec_2_tan
#   v_linetype    <- if_else(v_grid_interc == 0, 'solid', '13')
#   geom_vgrid    <- geom_abline(
#     aes(
#       intercept = v_grid_interc,
#       slope     = vec_2_tan,
#       linetype  = v_grid_interc == 0
#     )
#   )
# }
...
if (is.infinite(vec_2_tan)) grid_plot <- grid_plot + geom_vgrid
