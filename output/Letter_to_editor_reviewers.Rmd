...
To our understanding, a highly correlated latent space may imply a restricted dimensionality (as indicated by the rank of the covariance matrix) of the latent space.
We consider that this example, being an edge case (of unidimensionality), is very illustrative for our purposes, because it allows the reader to understand easily that all items should be aligned with the only dimension being measured and thus that grasp the intuitive notion that all direction cosines should be expected to be equal to 0.
However, in doing so, we have skipped a step in our reasoning, assimilating the dimensionality of the latent space with the dimensional sensitivity of the items.

To clarify better this point, we have tried to express better this distintion, emphasizing that the dimensionality of the latent space is an upper limit of the dimensional sensitivity of the items.
...
implying that the (alleged) three dimensions collapse into a single one.
This in turn makes the latent space unidimensional in practice.
As the dimensional sensitivity of the latent space sets the upper limit of
the dimensional sensitivity of a test measuring those traits,
it follows that the test must be unidimensional.
In such a case, when all the items are strictly aligned with
the one dimension they measure,
