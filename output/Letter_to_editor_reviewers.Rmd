...
Please refer to our 10th response to Reviewer 3 on p. <!-- TODO: Complete --> of this letter.
...
Although they attribute this to a possible "degree of confounding among errors of parameter estimation and scale
transformation", their groups 2 to 5 have all correlations equal (or almost equal) to .5, as it happens.

We believe this is also what happens when MIRT software such as NOHARM tries to force a correlated latent trait structure into its default orthogonal solution, as this editor himself has noted before [@ackerman_multidimensional_2005-1, p. 11].
While we consider this to be out of the scope of the current paper, we will gladly add further details to explain why these results are contradictory and how our method may solve the problem.
...
such as flexMIRT [@cai_flexmirt_2024] or R package {mirt} [@chalmers_mirt_2012],
...
We have thus made public [our GitHub repository for the manuscript](https://github.com/DaniMori/mirt-parameters) and [indexed it in Software Heritage](https://archive.softwareheritage.org/swh:1:dir:3914435e67a2b34847e36a712a181d7d9d76f35e;origin=https://github.com/DaniMori/mirt-parameters;visit=swh:1:snp:9069b256471ea08d5748e8bb021b7ee876aee724;anchor=swh:1:rev:fe49f438bf940606ae7e3c86c3c55f202e6920af) for being identified with a SWHID.
can do so if the journal accepts linking an external repository as supplementary material, or otherwise grant access to the editor and reviewers to this repository.
...
Therefore, we do not have any hypothesis to test, but the results of the formal derivations that we present, which address the objective described at the end of our introduction.
...
Please refer to our answer to the editor on p. 3 of this letter for more details on this addition.
...
the third paragraph of our Discussion, which now reads:
...
In this regard, we would kindly refer you to our third response to your comments, on p. <!-- TODO: Complete --> of this letter, where we discuss the representation of person parameters in the last two paragraphs.
...
This is the expression of a vector represented in a basis, in this case, representing that the vector can be represented in each of the two bases interchangeably.
...
This is a mistake on our side, we are afraid to admit, product of a careless translation of a false friend from our mother tongue Spanish ("indistintamente"), which means "indifferently".
We have changed the term by the mathematically appropriate "interchangeably" in our new version.
...
This is implicit in the definition of the test space itself (see p. 7 line 8 in our original submission, or p. <!-- TODO: Complete --> line <!-- TODO: Complete --> in the new version), and is independent of the basis chosen, which does not necessarily need to be a geometric representation of the covariance structure, as we argue in our introduction.
...
To our understanding, one may assume a theoretical dimensional sensitivity of a test space that is higher than the dimensionality of the latent space.
We acknowledge we may have overlooked this distinction.
...
From the model formulation, one can note that the components of the discrimination parameter serve as a scaling factor on the effect each latent trait has on the response probability function.
...
We believe this is the reason why estimating such a model would yield "a first dimension with high positive aparameters and other dimensions with much lower a-parameter estimates with many around zero" [@reckase_multidimensional_2009, p. 189].
...
However, in doing so, we may have skipped a step in our reasoning, assimilating the dimensionality of the latent space with the dimensional sensitivity of the items, as Reviwer 3 points out.

To clarify better this point, we have tried to express better this distinction, emphasizing that, in this case, the unidimensionality induced by the rank-limited covariance matrix restricts the dimensional sensitivity of the test space.
...
which in turn resticts the dimensionality of any response dataset
