...
Please refer to our response to Reviewer 3 on p. 18 of this letter.
...
As the editor says, the basis used to represent the test space changes its metric, which is determined by the Gram matrix ($`r INNER_PROD_MATRIX_INV`$, in contrast to $`r INNER_PROD_MATRIX`$ in the latent space).
...
Please see our next answer for further clarification on the interpretation of the item parameters, as well as our response above, on pp. 2-3 of this letter, for an example of how the interpretation of the item parameters may change when considering the covariance structure.

###### What implications does your approach have for standard psychometric procedures like equating.

It is our understanding that both common item design and common person design procedures can be applied soundly under this approach.
Despite the latent and the test space being (generally) represented in a different geometric basis, note that if one of them is known, the other one is determined.
This linkage between the two spaces, along with the model invariance, implies that any transformation in one space must be accompanied by the corresponding transformation in the other.
These relationships allow for both common-item and common-person design equating procedures, derived according to Reckase [-@reckase_multidimensional_2009, see Chapter 8], and do not change in our approach.
For instance, if the common item-design procedure is used (where the item parameters in Form 2 of a test are transformed to resemble as closely as possible those in Form 1), both the item parameters and the covariance matrix will be affected.
Although it is out of the scope of our paper to explore in depth the implications for these psychometric procedures, we have added a new paragraph to our discussion briefly explaining this fact and acknowledging our limitation.
This new paragraph reads:
...
Regarding the multidimensional parameters, it is clear that different covariance matrices may have a substantial effect on the multidimensional parameter values.
This is relevant for multi-group procedures because, even if the item parameters are equal, if the covariance structure differs across groups, the multidimensional parameters will not be the same.
...
Although it could be interesting to explore whether it is more realistic to assume equal M2PL parameters across groups (and thus possibly different multidimensional parameters) or vice-versa, we believe that this is an empirical question that also lies outside the scope of our current manuscript.

We have probably made the mistake of assuming that this was implicitly understood, for which we apologize to the editor.
To improve and address this point better, we have made some additions to the second paragraph of our "Discussion" section (p. 30, lines 5-9 in the original submission, or p. 31, lines 1-12 in the new one), which now reads:
...
We have added a short reference to them in the last paragraph of our introduction (p. 6, lines 10-13), to make sure their pertinence as examples is clear.
...
In this respect, please see especially the responses to the editor in pp. 4-7 in this letter.
...
Our intention in Equations (26) and (27), numbered (27) and (28) in the new version, was to express the item vector coordinates in terms of the original model parameters directly.
To do that, we have made use of the equality $`r DISCR_VECTOR_COORDS_EQ`$, stated on p. 7, or p. 8, line 23 (third by the end) in the new submission.
...
To correct this, we have modified the immediately preceding paragraph to the following (please note that the equation numbering has also increased by 1):
...
Please refer to our answer to the editor on pp. 2-3 of this letter for more details on this addition.
...
However, please bear in mind that this transformation in the latent space is a reasoning artifact to obtain the proper expression of the item parameters in the general case; for further explanation about this, please see our response to the editor in pp. 4-5 of this letter.
We have tried to convey this when explaining the rationale behind the covariance-based version of the parameters (see p. 14, lines 15-22, or p. 15, line 22, to p. 16, line 2, in the new version).
...
the third paragraph of our Discussion, which now reads (p. 31, line 22, to p. 32, line 1):
...
In this regard, we would kindly refer Reviewer 3 to our response on pp. 15-16 of this letter, where we discuss the representation of person parameters in the last two paragraphs.
...
This is the expression of a vector represented as a linear combination of
the vectors that make up the basis in which it is represented
(with the coefficients $`r TRAIT_VEC_COMP`$ and $`r TRAIT_VEC_ORTH_COMP`$ being
the scalars corresponding to each basis).
In this case, we intend to state that the vector can be represented in each of the two bases interchangeably.
...
This is implicit in the definition of the test space itself (see p. 7 line 8 in our original submission, or p. 8, line 12 in the new version), and is independent of the basis chosen to represent the items.
This basis does not have to be necessarily a geometric representation of the covariance structure, as we argue in our introduction, so it can be the canonical basis, or any other, orthonormal or not, and not necessarily representing the covariance structure.
...
We have added this latter clarification in our new version (p. 34, lines 13-16):
...
As we mention above, when the attributes are correlated, a distance measure that keeps the norm invariant in the latent space is the Mahalanobis distance (please see our response on pp. 16-17 in this letter).
...
This is explained in our manuscript, on p. 19 (lines 9-14) in the original version, and on p. 20 (last paragraph) in the new submission.
...
The aforementioned paragraph now reads (p. 29, lines 2-10):
...
In this case, the basis has been chosen to be a geometric representation of the correlation matrix (as given by the expression $`r INNER_PROD_CORR_COND`$, please see p. 20, last two paragraphs, in the new version), and therefore, the scales are not affected (only the relative directions among the axes).
...
We have found this comment puzzling as, to our understanding, what Reviewer 3 is asking is what we do in Figure 1 (p. 22 in our original submission, p. 23 in the new one).
...
We acknowledge, though, that this may have been explained without enough clarity, and therefore we have striven to improve this discussion.
The aforementioned paragraph now reads (p. 33, lines 16-19):
