...
In addition to the previous changes, we have made a minor addition in the "Covariance-based version" section, to refer to the transformation that turns the latent trait distribution into a standard normal by its usual name in the literature, i.e., a whitening transformation.
...
(Please also note the update in the title page to acknowledge the colleague who brought this previous literature to our attention, as well as the valuable contributions of the reviewers.)
...
Please see the new paragraphs added in third and fourth place in section "Graphical Representation Example" (p. <!-- # TODO: Complete -->, ll. <!-- # TODO: Complete -->):

> In panel (b), the item coordinates are computed from
> the agnostic version of the parameters.
> These coordinates are the same ones as in the first case, but
...
> Panel (c) shows the items with their coordinates computed using
> the correlation-based version of the parameters. [...]
>
> Comparing it to panel (b), we can see that the item coordinates are in general
> not the same as the ones computed with the agnostic version of the parameters;
> this effect is especially salient in items 2 and 3,
> which are close to the 90% contour in the agnostic version,
> but are almost along the 50% contour in the correlation-based one. [...]
...
> in the multidimensional metric,
> in two different cases with different latent covariance structures.
> In the first case (columns $`r CORR_ORTH_OUT`$),
> the two latent dimensions are independent;
> in the second one (columns $`r CORR_ORTH_OUT`$),
> the correlation between the dimensions is .5.
> The same `r MODEL_ACRONYM` parameters are used in both cases,
> but their (correlation-based) multidimensional parameters change when different
> correlations are used.
> 
> Panel (a) of Figure 1 shows
> the item representation in the uncorrelated case,
> while the correlated case is shown in panels (b) to (d)
> under different conditions.
> In all the four panels, the contours of a bivariate standard normal density
> with the corresponding correlation are plot along with the items
> (from outer to inner, the contours are the `r contour_breaks_out` of
> the density at its mode).
> Note that the stretched ellipses in panel (c) are the result of transforming
...
> the test space is transformed inversely as in the latent space, that is,
...
> Finally, panel (d) illustrates the effect of plotting the coordinates directly
> on the canonical basis; the relative positions and distances are correctly
> represented (which allow a more direct comparison with panel (b)),
> but the angles of the items with the axes are distorted:
> For example, item 1 forms 45-degree angles with both axes,
