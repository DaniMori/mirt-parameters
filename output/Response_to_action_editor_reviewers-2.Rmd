...
We understand this graphic may be misleading, but the grid lines are properly placed in our version of the figure:
Each point in the test space is intended to represent a coordinate vector, which is the displacement from the origin given by a linear combination of the basis vector, with the vector components being the coefficient of each basis vector.
For example, the tip of the vector representing item 1 is at coordinate (+2, +2).
This means the point is displaced twice the length of the first vector basis along its direction (i.e., along the horizontal axis), that is, two units along the horizontal axis, and then another _two units parallel to the direction of the oblique axis_.
These displacements, parallel to the direction of the axess, is what the grid lines intend to represent.
This representation is somewhat equivalent to a _pattern matrix_ representation in the factor model, while the one proposed by the reviewer would stand for the correlation of the item with each latent dimension, being somewhat equivalent to a _structure matrix_ representation.
This can be more easily understood by comparing figures <!-- # TODO: complete --> and <!-- # TODO: complete --> in @harman_modern_1976 <!-- # TODO: complete -->.
