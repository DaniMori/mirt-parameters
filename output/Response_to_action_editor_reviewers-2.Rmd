...
In p. 12 (line 8), simply dropping "already" made the sentence lose the intended
meaning that the implicit assumption of orthogonality in @reckase_difficulty_1985 is already present in Equation 3, and not just from Equation 7 (p. 404) on, as one might think in the first place.
We deem this point important because it took us quite some effort to notice that generalizing the transformation to polar coordinates to non-orthonormal bases implied reformulating this equation.
In an attempt to keep the intended meaning while improving the sentence structure, we have restated it as (p. 12, ll. 3-4):

> [...] this shows that the orthogonality of the latent space in @reckase_difficulty_1985 is assumed as early on as in Equation 3 (p. 403) [...]

In p. 13 (line 2), dropping "up", i.e., "the squared direction cosines sum ~up~ to 1", did not sound well to us, so instead we reformulated the sentence as (p. 12, ll. 20-21):
...
Finally, the sentence that was found difficult to understand in the abstract (p. 1, ll. 5-7) has been rewritten as follows (p. 1, ll. 4-7 in the current version):
...
After the changes, this paragraph (now in p. 41, l. 4 to p. 41, l. 3) reads:

> We have seen that taking the latent space structure into account may have a
> substantial effect on the resulting values and
...
> For example, using the agnostic version of the parameters
> and/or an orthogonal plot,
...
The second change in the discussion consists of stating this explicitly now at the end of the fourth paragraph (previously the fifth one) of the "Discussion" section, in p. 40, ll. 1-3:
...
Besides the aforementioned changes and some minor corrections, we would like to highlight two additional changes: a correction, and a minor addition to the "Covariance-based version" section.
The first one corrects ll. 16-18 in p. 19 in the previous version, which stated:

> the sign of $`r DISTANCE_PARAM`$ is equal to
...
This was an error, as the signs of $`r DISTANCE_PARAM`$ and $`r INTERCEPT_PARAM`$ are opposite to each other.
With this correction applied, these lines now read (p. 20, ll. 16-18):
...
In the "Covariance-based version" section, we have referred to the transformation of the latent trait distribution into one with an identity covariance matrix as  a "whitening transformation", its usual name in the literature (unknown to us before).
The second paragraph of this section (p. 16, ll. 16-19) now reads:
...
We had overlooked labeling the axes and have done so in this new version;
...
This choice of labeling is explained in the text in p. 23, ll. 4-11 (see also our response to the last comment below):
...
Please see the new paragraph added in third place in section "Graphical Representation Example" (p. 24, ll. 11-17):
...
Each point in the test space is intended to represent a coordinate vector, which is the displacement from the origin given by a linear combination of the basis vectors, with the components of the coordinate vector being the coefficients of those basis vectors (please refer to the expression $`r DISCR_COORDS_ALT_EQ`$ in p. 8 of the previous version of our manuscript, or p. 9, l. 1 in the new submission).
...
This paragraph (p. 14, l. 16 to p. 15, l. 4) now reads as follows:
...
Subsequently, we have updated ll. 14-15 in p. 15 (now ll. 17-18) as follows:

> [...] Hence we may define the $`r MIL_PARAM`$ and $`r MDISC_SYM`$ using
> Equations (20), (21), and (25). [...]
...
This paragraph now reads (p. 21, l. 25 to p. 22, l. 4):
...
Starting in the first paragraph (p. 22, l. 24 to p. 25, l. 6), this section now reads:

> [...] The example in Figure 1 showcases
...
> $`r TRAIT_COORD_1`$ and  $`r TRAIT_COORD_2`$.
> 
> The parameters of the items in Figure 1 are shown in Table 1,
...
> given by the values under $`r CORR_ORTH_OUT`$,
...
We have briefly explained this in p. 24, ll. 17-25 (please refer to the changes explained above).
