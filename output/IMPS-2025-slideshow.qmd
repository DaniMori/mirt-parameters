...
::: notes
However, it is not uncommon to find cases like these in the literature, where the items are not monotonically increasing, and the authors do not suggest to "generalize these definitions".

On the contrary, what we find are these kind of downplaying comments: "particularly puzzling", or "as an examinee’s ability increases, the chance of answering the item correctly actually decreases" (asserting the obvious).

That is why we propose to relax the "increasing monotony assumption", considering items with "constant monotony" instead.
This includes all items that do not have a latent trait "optimum" to give a positive response, and then decrease in both directions, but rather that the more extreme the trait level, the more extreme the probability (positive or negative) as well, in either direction.
And this general case also includes the "constant probability" case (as "constant probability" implies no change, so "constantly null monotony").

This regarding so-called "inverse" items.
Quite straightforward.
Now, to something also necessary, but not so straightforward: The orthogonality assumption.
:::
