...
And second, this constrain you see there, which implies that the item is represented in an orthogonal basis. This is necessary for applying the "general Pythagorean theorem" as we saw before.

These assumptions derive from the traditional application of the model to the cognitive domain, but we want to apply multidimensional IRT to the non-cognitive domain as well, e.g. to personality measures.
...
In the multidimensional case, this may imply a mix of positive, negative, and null discrimination parameters.
:::
...
::: notes
The generalization is straightforward: The MDISC implies squared values of the discrimination components, so it is always positive and not affected by the sign of the discrimination components.

\[NEXT\]

The signed distance is also the same,...

\[NEXT\]

...and the direction only needs to take into account the sign of each component when computing the corresponding cosine.

\[NEXT\]

Graphically, we can see that each item points in the direction given by its discrimination components.
In these case, all of them have an absolute value of 1, so the direction is given by the sign of the two components.
:::
...
::: notes
Also, with non-null intercepts, the distance is still proportional to its absolute value, and the direction is again given by its sign and the direction of the vector (forward for negative intercepts, backward for positive intercepts).

So, we can see that this generalization is quite straightforward.
However, remember that Reckase explicitly constrained the parameter definitions to items that were "monotonically increasing", not simply "with constant monotony".k
:::
