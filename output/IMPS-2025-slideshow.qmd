...
::: notes
The Multidimensional 2PL model is one of many extensions of the 2PL model.
...
(This here is a vector inner product; the "T" means that the discrimination vector is transposed).

And it has this additional "intercept parameter"; so it is basically a logistic multivariate regression with latent predictors.
...
::: notes
The problem with the M2PL is that its parameters do not have a clear interpretation in IRT terms.
...
This discrimination can be used to define the item difficulty, finding a parameterization that parallels the unidimensional 2PL model.

Note though that, in this case, the difficulty has two components: a "signed distance" from the origin, and a direction.

\[NEXT\]

It is easier to appreciate this with the graphical representation, that was proposed by Ackerman a few years later, where we can see that the item vector is applied at a point given by the difficulty (with its distance and direction), and has a length equal to the discrimination.
The vector direction always goes through the origin, and the sign of the distance (positive in this case), which is the opposite of the intercept sign, will determine whether the vector is displaced forward or backward along that direction.
:::
...
::: notes
However, there are two assumptions for defining the multidimensional parameters.

First, that the probability of a correct answer increases monotonically with each dimension.

And second, this constrain you see there, which implies that the item is represented in an orthogonal basis.
This is necessary for applying the "general Pythagorean theorem" as we saw before.

So, our purpose is **to relax these two assumptions**.
:::
...
::: notes
Because they derive from the traditional application of the model to the cognitive domain, but we want to apply multidimensional IRT to non-cognitive items as well, e.g. to personality measures.

\[NEXT\]

Here, we can have items with "decreasing" probability.

\[NEXT\]

Formally, what this would imply is a negative discrimination parameter.

In the multidimensional case, this may imply a mix of positive and negative discrimination parameters, even in the same item.
:::
...
::: notes
The generalization is straightforward: The MDISC implies squared values of the discrimination components (this is the inner product expanded), so it is always positive regardless of the sign of the discrimination components.

The signed distance is also the same, and the direction only needs to take into account the sign of each component when computing that corresponding cosine.

\[NEXT\]

Graphically, we can see that each item points in a direction given by its discrimination components, including their signs (so, 1 1, 1 -1, etc.).

And the displacement from the origin is again with respect to the pointing direction.

This seems obvious enough, but we must explicitly state that this definitions are valid in these cases, and that they have the same interpretation.
...
¿Why? Because we want this space to have a meaningful interpretation; we want it to be a geometrical representation of the probabilistic properties of our sample.
:::
...
::: notes
So, let's say $\theta$ is multivariate normal with covariane matrix $\mathbf{\Sigma}$.
...
And if we want the geometrical space to be a representation of the latent space, the meaningful way is to represent these coordinates in an orthonormal basis (the covariane matrix is an identity matrix, the basis also forms an identity matrix).
:::
...
::: {.fragment fragment-index="2"}
Which coordinate system *θ* makes *θ\** "spherical"?
:::
...
...what is the coordinate system of $\mathbf{\theta}$...
...
Take a few seconds to answer and we will see next which is the correct one.
:::
...
::: notes
This is actually not new, it was already noted by Zhang & Stout, more than 25 years ago, to solve a different problem, but unfortunately they didn't provide a deduction as we do here.

But the important thing here, is that we have two different spaces: One for the latent traits and one for the items.

And knowing these...
:::

## Generalized MIRT parameters {.smaller}
...
plot_oblique +
  scale_color_manual(values = ITEMS_PALETTE, guide = NULL) +
  theme(axis.ticks = element_blank()) ## TODO: Tema ggplot
```
:::
:::::

::: notes
The complete definition and representation of the parameters is as you see here.
