...
...and the direction only needs to take into account the sign of each component when computing that corresponding cosine.

\[NEXT\]

Graphically, we can see that each item points in a direction given by its discrimination components, including their signs.
:::
...
::: notes
And the displacement from the origin is again given by the sign of the signed distance (so, opposite to the intercept sign).
:::
