...
algebraic sense in an arbitrary basis, and then defining such basis to be a
...
Fourth, $`r DISCR_VECTOR`$ is expressed on an algebraic basis,
which the MIRT literature implicitly assumes to be the canonical basis
(we will show that this assumption is not necessary though).
...
thus the test unidimensional in practice.
...
computed in that case in the table in Online Resource 1.
...
Furthermore, note that, when using the covariance-based version formulation,
