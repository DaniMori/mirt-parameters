...
\begin{equation}
  `r TRAIT_COORDS_ORTH_EQ`                    (\#eq:trait-coords-orth-spaces-eq)
\end{equation}
...
By \@ref(eq:change-basis), \@ref(eq:discr-vector-std),
...
both in the original `r MODEL_ACRONYM` metric, and in the multidimensional
metric in two different cases, with different latent covariance structures.
...
Note that, although the `r MODEL_ACRONYM` parameters are the same in both cases,
their (correlation-based) multidimensional parameters change when different
...
according to \@ref(eq:trait-coords-spaces-transform).
...
we have defined $`r TRANSFORM_MATRIX_TRANSP_INV`$ such that
