...
could only mislead researchers to wrong conclusions.
...
and we have seen that this can only be achieved
with a proper geometrical representation.
Although simple items are not affected much by this issue,
...
Our results will be especially relevant in such cases,
where the item interpretation may be more affected.
For example, the relative importance of an item in scoring a given trait
could be downplayed if it is strongly aligned with that dimension
but this alignment is overlooked.
Moreover, in multi-group applications,
