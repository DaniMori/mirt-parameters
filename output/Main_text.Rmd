...
output:
  officedown::rdocx_document:
    ...
    fig_height:      4
abstract: |
  Modern measurement theory demands that
  Multidimensional Item Response Theory models accurately represent
  covariance spaces with correlated dimensions and/or non-standard variances.
  As they are applied in the non-cognitive domain as well,
  they also need to model negatively-keyed items accurately and interpretably.
  Henceforth we generalize the multidimensional discrimination
  [@reckase_discriminating_1991] and difficulty [@reckase_difficulty_1985]
  parameters to account for those two phenomena.
  We do so by applying Reckase's [-@reckase_difficulty_1985] method
  ...
  Importantly, we find there is a test space where
  the items should be properly represented, which must be differentiated from
  the latent space itself.
  Following @ackerman_creating_1994, we also provide a geometrical
  representation based on these parameters in the test space.
  We apply our results to examples from the literature
  to illustrate the pertinence of these generalized parameters to get an
  accurate representation of the measurement properties of the items.
  We recommend using the version that uses the latent covariance structure
  for describing the properties of the parameters,
  and the one based on the correlation structure for graphical representation.
  Finally, we discuss the implications of this generalization for
  other multidimensional IRT models
  and the parallels of our results in common factor model theory.
keywords: |
  Multidimensional IRT, 2-Parameter Logistic model,
...
library(ggplot2)
library(rstudioapi)
library(rsvg)    # Used by {knitr} and {ggplot2} to render the figure to SVG;
library(svglite) #   added to prevent {renv} from reporting inconsistent state.
...
being the model interpretation secondary in relevance.
...
ggsave(file.path(DOC_DIR, "Figure_1.svg"))
```
