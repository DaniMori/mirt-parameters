...
- Daniel Morillo-Cuadrado. Statistical and Computational Methods in Psychology
  Group, Department of Behavioral Science Methodology, School of Psychology,
...
Email address: [danielmorillo.ac@gmail.com](mailto:danielmorillo.ac@gmail.com)
...
[swh:1:dir:f0a8de9085b7b72cd04ffb4a9a9c245a84ab5179;origin=https://github.com/DaniMori/mirt-parameters;visit=swh:1:snp:076d4a764ef2bbd1d961cccff56b76c59f8aa81e;anchor=swh:1:rev:11084a02d8365a878fe2135f46a459ce7044b4b2](https://archive.softwareheritage.org/swh:1:dir:6195936c6e16023c4a0b6a4500b81a34469d64f1;origin=https://github.com/DaniMori/mirt-parameters;visit=swh:1:snp:410cf1a6fb4dd4c10bdf0ce7f12f6890f560549d;anchor=swh:1:rev:1f1a708a7477dd61c0ba5c82dcf1d8523ab3d8ff)
...
to account for nonidentity latent covariances and negatively keyed items.
We apply Reckase's maximum discrimination point method to define them in an
arbitrary algebraic basis.
Then, we define that basis to be a
...
---that is, its discrimination parameters are equal with value $`r DISCR_SYM`$.
Applying Reckase & McKinley's [-@reckase_discriminating_1991] formula,
we would find that $`r AG_MDISC_EXAMPLE`$.
Assume that we apply this item in two groups where, taken to the extreme,
...
and (possibly) nonstandard units,
...
nonstandard latent dimensions and nonnull covariances among them
(a more realistic case than an identity matrix).
Assuming orthonormality among traits is usually unrealistic, be it in the
cognitive [e.g., @reckase_multidimensional_2009] or the noncognitive domain
[e.g., @thielmann_comparability_2022].
Moreover, certain applications impose the estimation of
nonidentity latent covariance matrices (e.g., multi-group equating) even when
...
literature often dismisses them as "particularly puzzling" (p. 109)
or implicitly assumes item malfunctioning without further discussion
[e.g., @ackerman_multidimensional_2005-1].
In the noncognitive domain though, the use of so-called negatively keyed items
...
is given by a nonpositive discrimination parameter
...
items with monotonically nonincreasing probabilities, as we will see.

The main objective of this paper is thus to provide a generalization of
the MIRT parameters in two aspects: first, in an oblique coordinate system
that can be soundly applied to the correlated trait case, and second, to
...
These examples, from the cognitive and the noncognitive domain, respectively,
...
nonnegative real numbers in @reckase_difficulty_1985
...
if the $`r DISCR_VECTOR`$ parameter only has one nonnull component.
...
Let us consider an orthonormal basis $`r LS_ORTH_BASIS_EQ`$ in
$`r LATENT_SPACE`$ with the dot product as inner product.
...
Let us consider now the vector $`r DIR_COS_VEC_ORG`$, made up by
the $`r N_DIMS`$ direction cosines of $`r TRAIT_VECTOR`$ with
...
Therefore, $`r DIR_COS_VEC_EQ`$ and
$`r TRAIT_VECTOR_POLAR_EQ`$.$`r END_OF_PROOF`$
:::
...
::: {custom-style="First Paragraph"}
**Proposition 2** *Let us consider another basis $`r ALT_BASIS_EQ`$ in
...
Let us call $`r TRAIT_COORDS_ALT_EQ`$ and let $`r TRANSFORM_MATRIX_ALT_EQ`$
...
Importantly, this shows that the orthogonality of the latent space in
@reckase_difficulty_1985 is assumed as early on as in Equation 3 (p. 403),
as we stated before.
...
Reckase leverages the property that the sum of the squared direction cosines
equals 1 (see Equation 7 therein, p. 404).
...
\begin{equation}
  `r TRAIT_COORDS_ORTH_SPACES_EQ`             (\#eq:trait-coords-orth-spaces-eq)
\end{equation}
...
by Equations \@ref(eq:change-basis), \@ref(eq:discr-vector-std),
and  \@ref(eq:trait-coords-orth-spaces-eq),
any coordinate vector $`r TRAIT_COORDS`$ in $`r LS_BASIS`$
can be expressed in $`r TEST_SPACE_BASIS`$ as

\begin{equation}
  `r TRAIT_COORDS_TS_EQ`,                   (\#eq:trait-coords-spaces-transform)
\end{equation}
  
and vice versa.
As we can see, the invariance of the model holds,
regardless of the space in which we represent the coordinates.
...
The multidimensional item location ($`r MIL_PARAM`$) is defined as
...
Equations \@ref(eq:distance-param), \@ref(eq:slope-param),
and \@ref(eq:dir-cos-vec-test-space).
...
which implies that Equation \@ref(eq:change-basis) actually represents
...
To do this, let us assume a matrix $`r TRANSFORM_MATRIX_AUX_EQ`$, such that
...
$`r MDISC_SYM`$ is strictly positive for nonnull vectors.
Therefore, the sign of $`r DISTANCE_PARAM`$ is the opposite of
the sign of $`r INTERCEPT_PARAM`$.
The implication is that the item is displaced along its measurement direction
forward or backward with respect to the origin, depending on
whether $`r INTERCEPT_PARAM`$ is negative or positive, respectively.
...
the parameters to the monotonically nonincreasing case:
...
Also, in the covariance-based version,
...
of our general, possibly nonrectangular coordinates.
...
The example in Figure \@ref(fig:item-plot-out) showcases
the test space representation of a set of items
under two different latent trait distributions.
In formal terms, the proper axis labeling of this figure
should correspond to the vectors that make up basis $`r TEST_SPACE_BASIS`$,
in this case, $`r ITEM_BASIS_VECTOR_FIRST`$ and $`r ITEM_BASIS_VECTOR_2`$;
the coordinates represent the coefficients that multiply each basis vector to
obtain the corresponding coordinate in the test space.
However, these coefficients correspond to the values of
a vector in the test space, be it $`r DISCR_COORDS`$ for $`r DISCR_VECTOR`$,
or $`r TRAIT_COORDS_TRANSF_TS`$ for $`r TRAIT_VECTOR`$
(as given by Equation \@ref(eq:trait-coords-spaces-transform)).
For the sake of simplicity,
we label the axes as $`r TRAIT_COMP_1`$ and $`r TRAIT_COMP_2`$,
highlighting the comparison of the items with the latent trait vectors.
However, note that these coordinates actually correspond to
the components of $`r TRAIT_COORDS_TRANSF_TS`$, i.e.,
$`r TRAIT_COORD_1`$ and  $`r TRAIT_COORD_2`$.

The parameters of the items in Figure \@ref(fig:item-plot-out) are shown
in Table \@ref(tab:example-items-table-out),
...
Panel (a) of Figure \@ref(fig:item-plot-out) shows
...
according to Equation \@ref(eq:trait-coords-spaces-transform).
...
#|   with the correlation-based ones, which are then plotted either
...
a bivariate standard normal distribution, with null correlation (a),
or with correlation $`r CORR_OBL_OUT`$ (b-d).
The contour plots represent (from outer to inner)
the `r contour_breaks_out` of the maximum density.
:::
...
when Table \@ref(tab:example-items-table-out) and panel (c) show they are not,
and item 1 forms 45-degree angles with both axes
instead of the 30-degree ones shown in Table \@ref(tab:example-items-table-out).

A comparison of panels (a) and (c) allows understanding the effect of
a positive correlation on the $`r MDISC_CORR_PARAM`$
(and consequently the $`r MIL_CORR_PARAM`$) value of an item:
...
the $`r MDISC_CORR_PARAM`$ value tends to increase;
on the contrary, when the discrimination parameters have opposite signs,
they tend to cancel each other out, and $`r MDISC_CORR_PARAM`$ decreases,
as happens with items 2 and 3.
This effect can be especially noticed by comparing
the $`r MDISC_CORR_PARAM`$ values of items 3 and 5:
their discrimination parameters are equal in absolute value,
so their $`r MDISC_CORR_PARAM`$ values are equal in the orthogonal space;
...
an item vector is applied at the origin when its value is null (item 1),
...
even when they are not parallel to the other one.
...
it pertains to the noncognitive domain and contains
...
dims_corr_out <- DIMS_CORR |> format_prop_like(sig = 1)
```
...
both of these two dimensions drastically change their multidimensional
...
(compared with the ones in dimensions 2 and 3, respectively)
are barely affected.
...
with that dimension.
In this case, because the discrimination in dimension 4 is so low,
the situation is very similar to item 4
in Table \@ref(tab:example-items-table-out) and Figure \@ref(fig:item-plot-out).
...
Applying the general procedure outlined by @reckase_difficulty_1985,
and later extended by @reckase_discriminating_1991,
...
and how this holds for any case of unchanging monotonicity
...
(although constrained to nondecreasing monotonicity).
...
although in many applications both versions will fortunately be coincident.
...
the procedure of @reckase_difficulty_1985 and @reckase_discriminating_1991.
...
However, the representation in the test space allows us to obtain
the direction cosines relative to the original latent trait axes,
which may be useful if they represent actual, substantive traits.
Computing the parameters in the orthonormalized (or just orthogonalized) space,
by contrast, would require transforming the cosines back to
the original, non-orthonormal basis.
...
and apply an oblique rotation afterward.
...
The latent space, where the person parameters are represented,
has the inverse of the covariance (correlation) matrix as
its inner product matrix, though.
...
on our understanding of multi-group MIRT applications,
which involve several latent spaces.
...
any transformation in any of them will imply
the inverse transformation in the other one,
thus preserving their relationship.
...
substantial effect on the resulting values and interpretation of the parameters,
...
For example, using the agnostic version of the parameters and/or an orthogonal plot,
the alignment of an item with the trait axes could be easily misinterpreted.
Then, the information it provides about each of those traits
(and, subsequently, its quality for assessing them)
could be either downplayed or exaggerated.
Once again, this is paramount in multi-group applications,
where differences in the latent space structure are critical and
the agnostic version could overlook relevant differences in
...
whereas noncognitive applications often (and sometimes cognitive ones as well)
