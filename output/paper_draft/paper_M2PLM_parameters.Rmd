...
We will consider now two more properties,
related to each of the two components of the $`r MIL_PARAM`$ parameter,
namely, the distance to the origin, and the measurement direction.
...
That is, the more discriminating the item is,
...
The most appropriate way of representing them graphically is
a vector plot [@ackerman_creating_1994].
This kind of plot allows representing several items altogether and analyze them
...
In the oblique case it is not so simple though, but we take advantage of the
...
This implies pre-multiplying its coordinates in basis $`r LS_BASIS`$
...
we want to avoid changing the scale of the parameters.
We will thus prefer the correlation-based over the covariance-based parameters.
Thus, we define $`r TRANSFORM_MATRIX_TRANSP_INV`$
to be a square root matrix of $`r CORR_MATRIX`$.
