...
Therefore, we find a basis $`r TEST_SPACE_BASIS`$ in $`r TEST_SPACE`$
defined by
...
with $`r TRANSFORM_MATRIX_TRANSP_INV`$ a change of basis matrix.
Note that, as $`r TEST_SPACE_STD_BASIS`$ is orthonormal,
$`r TRANSFORM_MATRIX_TRANSP_INV`$ determines $`r TEST_SPACE_BASIS`$.
...
We shall refer to this new space $`r TEST_SPACE`$ as
the *test space* [@zhang_theoretical_1999].
