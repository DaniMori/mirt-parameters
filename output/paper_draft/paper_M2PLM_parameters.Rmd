...
On the basis of these conditions, we will need to compute
...
as they are simply the orthogonal projections on the respective axes.
In the oblique case it is not so simple though, but we take advantage of the
orthornormalization introduced above.
In $`r TEST_SPACE_STD_BASIS`$ , the origin and end coordinates are
...
i.e., $`r ORIGIN_ORTH_EQ`$ and $`r END_ORTH_EQ`$.
Given the mapping of $`r TEST_SPACE_STD_BASIS`$ basis onto $`r LS_ORTH_BASIS`$,
Equation \@ref(eq:dir-cos-vec-std) gives
