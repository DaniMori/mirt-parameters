...
A respondent is represented in $`r LATENT_SPACE`$ by
coordinate vector $`r TRAIT_VECTOR`$, assumed to be multivariate normal:
...
a positive response to item $`r ITEM_INDEX`$ for a given $`r TRAIT_VECTOR`$,  
...
the second derivative with respect to $`r TRAIT_VECTOR`$.
...
which result in improper values of $`r TRAIT_MODULE`$).
