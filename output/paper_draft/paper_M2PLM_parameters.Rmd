...
the R package *psych*, <!--# Reference needed? -->graphically represents
...
Consider now the M2PL model formulation given by Equation
\@ref(eq:M2PL-formulation), on one hand, and Equation \@ref(eq:change-basis) on
the other.
Since the response probability must be invariant with respect to
a change of basis,
we must define the coordinates $`r DISCR_STD_COORDS`$ of the parameter vector
$`r DISCR_VECTOR`$ in an orthonormal basis $`r TEST_SPACE_STD_BASIS`$,
such that $`r INVARIANCE_EQ`$.
If we assume that $`r DISCR_VECTOR`$ exists in vector space $`r TEST_SPACE`$,
with coordinates $`r DISCR_COORDS`$ in basis $`r TEST_SPACE_BASIS`$,
then we must define $`r DISCR_COORDS`$ such that

\begin{equation}
  `r DISCR_STD_COORDS_EQ`                                (\#eq:discr-vector-std)
\end{equation}

We want @reckase_difficulty_1985's definition of the multidimensional parameters
in rectangular coordinates to be a particular case of our more general
definition, so we can conveniently assume that the inner product in
$`r TEST_SPACE_STD_BASIS`$ is the dot product,
as we did with $`r LS_ORTH_BASIS`$.
...
This shows that discrimination vectors are indeed represented in their own
vector space $`r TEST_SPACE`$ with basis $`r TEST_SPACE_BASIS`$.
We shall refer to this new space as the *test space* [@zhang_theoretical_1999].
Although the inverse relationship between their respective inner product
Gram matrices induces a bijective relationship between the two sets of spaces,
it is worth highlighting that test space $`r TEST_SPACE`$
and latent space $`r LATENT_SPACE`$ are different entities. Only in the
orthonormal case the two spaces share a common inner product
(i.e., the dot product), and thus can be mistaken.
...
As $`r DISCR_VECTOR`$ is defined in test space $`r TEST_SPACE`$ with basis
$`r TEST_SPACE_BASIS`$, its algebraic vector values correspond to its
coordinates, i.e., $`r DISCR_VECTOR_COORDS_EQ`$. Therefore, by Equation
\@ref(eq:discr-vector-std),
...
In the test space we have that
...
Now we may consider two different possibilities:
...
we want to avoid changing the scaling of the parameters.
We will thus prefer the correlation-based over the covariance-based indices.
