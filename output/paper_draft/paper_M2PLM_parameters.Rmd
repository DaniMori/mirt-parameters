...
```{r write-bib, cache=TRUE}
# automatically create a bib database for R packages
write_bib(
  c(.packages(), 'bookdown', 'knitr', 'rmarkdown', 'lavaan'),
  'www/packages.bib'
)
```
...
[M2PL, @mckinley_extension_1983] model is one of the most important and
widely used multidimensional models,
partly due to its close relationship to the common factor model
[@mcdonald_test_1999], but also to the formal tractability of its formulation.<!--# Ref. needed? -->
The M2PL model is a generalization of the 2-parameter logistic model
...
and a surplus in another one cancel out their effects --thus it is sometimes
referred to as a *compensatory* model.

@reckase_difficulty_1985 argues in favor of a need to characterize
the statistical properties of items taking dimensionality into account.
As such, he outlines a general procedure for determining
the difficulty of multidimensional items.
Following this procedure, he also derives
a *multidimensional difficulty* parameter (MID) for M2PL items.
...
the maximally discriminating point of the item.
The second one, the direction of the vector from the origin to that same point.
Following a similar rationale, @reckase_discriminating_1991 define
a *multidimensional discrimination* (MDISC) parameter definition for that model.

A crucial assumption leading to those formulations is that the axes of the
latent space are orthogonal
[see @reckase_difficulty_1985, p. 404 and Equation 7].
...
the two theoretical programs put the focus on;
...
modern measurement theory [@mcdonald_test_1999] imposes
...
is usually implicit in the MIRT literature without further consideration<!--# Exaples? -->
[see @zhang_theoretical_1999 for a notable exception].
What is more, disregarding this geometrical interpretation may lead to
misconceptions that put the measurement and its conclusions into question.<!-- completar?? -->

A latent space where the traits are correlated is more readily interpretable as
a geometrical space with non-orthogonal axes,
...
Assuming orthogonality among traits is usually unrealistic, be it in the
...
As we will argue, the usual definitions of the
MID and MDISC parameters do not generalize to the non-orthogonal geometry used
to represent correlated latent spaces.
...
are usually reverse scored-- is also widespread.<!--# Reference and example? -->
...
dimension is given by a non-positive discrimination parameter
...
the MIRT parameters in two senses: on one hand, in an oblique coordinate system
...
graphically representing these items in the fashion of @ackerman_graphical_2015,
but taking into account the non-orthogonality of the coordinate axes.
In order to pursue these objectives, we first define the generalization of
the M2PL model and describe its scope of application.
...
Next we discuss the properties of the indices,
propose a graphical representation method,
and present an example of computation<!-- # TODO: Pending -->
and representation of the items based on these parameters.
...
defined by vector space $V$ and basis $`r LS_BASIS`$,
being $`r LS_BASIS_EQ`$ an arbitrary set of linearly independent vectors.
Person $j$ is represented in $`r LATENT_SPACE`$ by vector
$`r TRAIT_VC_PERSON_J`$,
being $`r TRAIT_VECTOR`$ a multivariate normal variable
...
As in @reckase_difficulty_1985, the characteristic function of item
$`r ITEM_INDEX`$ according to the M2PL model is formulated as follows:
...
being  
$`r IRF_M2PL`$ ($= `r IRF_ABBR`$ henceforth, for simplicity) the probability of
a positive response to item $`r ITEM_INDEX`$ by person $`r PERSON_INDEX`$,  
$`r DISCR_VECTOR`$ a $`r N_DIMS`$-dimensional vector of
discrimination parameters, and  
$`r INTERCEPT_PARAM`$ an intercept parameter related to the location of the item
in the latent space.

We must make a few remarks here. First, as we are not constraining ourselves to
cognitive traits, we speak of _positive_ instead of _correct response_.
This may imply a correct response, if we are talking about a cognitive test, or
endorsing an item (or a certain response option) in a non-cognitive domain.
Second, although not explicitly stated, $`r DISCR_VECTOR`$ is a vector of
non-negative real numbers in Reckse
[-@reckase_difficulty_1985, a decrease in the probability of a correct response with an increase in ability makes little sense].
...
we need to introduce a few linear algebra results before proceeding futher.

## Linear algebra of non-orthogonal coordinates
<!-- # TODO: To appendix, OR citation about these? -->

### Inner product

Let $`r INNER_PROD_EQ`$ be the inner product in $`r LATENT_SPACE`$, with
the positive-definite matrix $`r INNER_PROD_MATRIX`$ its inner product matrix.
We may apply a change of basis in $`r LATENT_SPACE`$ that results in a space
$`r LATENT_SPACE_STD`$ with the standard basis $`r LS_STD_BASIS_EQ`$.
Such a change of basis can be defined as
...
with $`r TRAIT_VECTOR_STD`$ the coordinates
of $`r TRAIT_VECTOR`$ in $`r LATENT_SPACE_STD`$ and $`r TRANSFORM_MATRIX_EQ`$
(being $`r BASIS_VECTOR_ANY_TRANSF`$ the coordinates of
$`r BASIS_VECTOR_ANY`$ in $`r LATENT_SPACE_STD`$
for any dimension $`r DIM_INDEX`$).
This new space can be defined as a Euclidean space with standard inner product
(i.e. dot product), such that the norm $`r NORM_EQ`$ of $`r TRAIT_VECTOR`$
in $`r LATENT_SPACE_STD`$ is given by<!-- # TODO: Reference needed? -->

\begin{equation}
  `r TRAIT_MODULE`^2 = `r TRAIT_VECTOR_STD_SQ` =
    `r TRANSF_TRAIT_SQUARED` =
    `r TRAIT_TRANSF_TRAIT`.                                 (\#eq:inner-product)
\end{equation}

Thus, the inner product matrix $`r INNER_PROD_MATRIX`$ in $`r LATENT_SPACE`$
is consistently defined as

\begin{equation}
  `r INNER_PROD_MAT_EQ`.                            (\#eq:inner-prod-matrix-def)
\end{equation}

### Direction cosines

The cosines between two vectors follow the usual definition,
<!-- # TODO: Is this true?? See Harman 1970 (eq. 4.43, p. 60 to check) -->

\begin{equation}
  `r COS_VECTORS_EQ`,                                         (\#eq:cos-vectors)
\end{equation}

independently of whether the inner product is standard or not.
Therefore, to compute the direction cosine $`r DIR_COS_ANY`$
...
By Equation \@ref(eq:inner-prod-matrix-def) it is clear that
$`r DIAG_INNER_PROD_MATRIX_EQ`$
(but note that the basis vectors are not necessarily unitary).
By defining $`r DIAG_MATRIX_INNER_PROD`$ as
the diagonal matrix with the same diagonal as $`r INNER_PROD_MATRIX`$,
Equation \@ref(eq:direction-cosine) in matrix notation is<!-- # TODO:
Not sure if it’s necessary to explain this further.
There may be a certain “inferential leap” assuming that D^(-1/2) is
the matrix form equivalent to 1/u_k,
(but I “know” intuitively that the generalization holds) -->
...
From Equation \@ref(eq:direction-cosine-vector),
...
Thus, the M2PL model is expressed in polar coordinates as
...
the second derivative with respect to $`r TRAIT_J_MODULE`$.
...
which result in improper values of $`r TRAIT_J_MODULE`$).
The slope in direction $`r DIR_ANGLE_J_VEC`$ is given by

\begin{equation}
  `r IRF_1ST_DIFF_EQ`.                                      (\#eq:M2PL-1st-diff)
\end{equation}

When $`r IRF_MAX_SLOPE_EQ`$,
...
When $`r LS_BASIS`$ is the standard basis,
this term simplifies to an identity matrix
(which results in Equation \@ref(eq:M2PL-formula-polar) through
\@ref(eq:slope-max) simplyfing to rectangular Cartesian coordinates,
i.e. the equations outlined by Reckase).
...
However, this assertion does not apply to the general case,
as it implicitly assumes orthogonal coordinates.<!-- # TODO: To appendix the following, or maybe give for granted? -->
Instead, we may consider the direction cosine vector in $`r LATENT_SPACE_STD`$:

\begin{equation}
  `r DIR_COS_VEC_STD_EQ`.                     (\#eq:direction-cosine-vector-std)
\end{equation}

Operating on Equation \@ref(eq:trait-vector-polar),
...
and, substituting in \@ref(eq:slope-max),
...
such that Equation \@ref(eq:slope-max-std-cosines) is similar
(in vectorial form) to Reckase's [-@reckase_difficulty_1985] Equation 6,
and thus can be solved by the same procedure
applying their Equations 7 through 9.
The resulting equivalent to their Equation 9, in vectorial form
and solving for $`r DIR_ANGLE_STD_SUBSTITUTE`$ is

\begin{equation}
  `r DIR_COS_STD_ITEM_VEC_EQ`.                            (\#eq:dir-cos-vec-std)
\end{equation}

Substituting $`r DIR_COS_STD_ITEM_VEC`$ and $`r DISCR_VECTOR_STD`$
by their respective expressions results in the expression for
the direction cosine vector of item $i$ in $`r LATENT_SPACE`$,
...
we solve Equation \@ref(eq:M2PL-formula-polar) for $`r IRF_MAX_SLOPE_EQ`$,
$`r DIR_COS_SUBSTITUTE`$ and
$`r DISTANCE_SUBSTITUTE`$,
and then substitute Equation \@ref(eq:dir-cos-vec) for $`r DIR_COS_ITEM_VEC`$.
The result, solving for $`r DISTANCE_PARAM`$, is
...
Solving for $`r DIR_COS_SUBSTITUTE`$
and substituting Equation \@ref(eq:dir-cos-vec) for $`r DIR_COS_ITEM_VEC`$
[@reckase_discriminating_1991],
...
Equation \@ref(eq:slope-param) makes it clear that a space $`r TEST_SPACE`$
with inner product matrix $`r INNER_PROD_MATRIX_INV`$ can be defined,
such that $`r INNER_PROD_DISCR_EQ`$ and, subsequently,
$`r DISCR_VECTOR_MODULE_EQ`$.
We call $`r TEST_SPACE`$ the *test space* [@zhang_theoretical_1999] of the
...
as their inner product matrices are inverse of one another.

The basis $`r ITEM_SPACE_BASIS_EQ`$ of $`r TEST_SPACE`$ is such that
$`r ITEM_TRANSF_MATRIX_EQ`$ (see Equation \@ref(eq:discr-vector-std)).
Thus, similarly to equation \@ref(eq:dir-cos-vec),
the vector of direction cosines of a vector in $`r TEST_SPACE`$
can be defined as
...
with $`r DIAG_MATRIX_INNER_PROD_INV`$ the diagonal matrix with
the same diagonal as $`r INNER_PROD_MATRIX_INV`$.
...
By analogy with the unidimensional case, $`r MDISC_PARAM`$ is defined as
$4 `r SLOPE_MAX_PARAM`$ [@reckase_discriminating_1991].
Therefore, we may define the $`r MIL_PARAM`$ and $`r MDISC_PARAM`$ using
...
3. $`r MDISC_PARAM`$ is four times the maximum slope $`r SLOPE_MAX_PARAM`$.

The second property derives straightforwardly from
Equation \@ref(eq:distance-param),
and the third one is implicit in the definition of the $`r MDISC_PARAM`$.
...
$`r MDISC_PARAM`$ is strictly positive for non-null vectors.
...
also due to the strictly positiveness of $`r MDISC_PARAM`$,
...
orthogonality assumption, but will depend on the correlation matrix
...
and will have a length equal to its $`r MDISC_PARAM`$ parameter.
...
In the second one, the correlation between the dimensions is $`r corr_out`$.
...
The sample items have the same M2PL parameters in both latent spaces.
...
along with their corresponding M2PL parameters,
...
```{r example-items-table-out, cache=FALSE, tab.id="example-items-table-out", tab.cap="Item parameters for the graphical example"}
item_params_output
```
...
```{r item-plot-out, cache=FALSE, fig.cap="Item plots in an orthogonal (left) and oblique (right) test space.", fig.height=3.1, fig.width=6.65}
...
with a positive increase in the correlation.
Thus, if their discrimination parameters have the same sign, like items 1 and 5,
their $`r MDISC_CORR_PARAM_ITEM`$ value tends to increase;
on the contrary, when the discrimination parameters have opposite sign
they tend to cancel each other out, as happens with items 2 and 3.
...
so its $`r MDISC_CORR_PARAM`$ is unaffected by the correlation.

We can also see how the sign of $`r INTERCEPT_PARAM`$ affects the item distance:
The item is applied at the origin when its value is null (item 1),
and tends to be displaced
...
Finally, we can see how the item direction is determined by the discrimination
parameters *and* the correlation; the sign of the discrimination parameters
determine the quadrant where the item will point at;
thus, an item with two positive (negative) discrimination parameters will be
in the first (third) quadrant,
...
of the $`r MDISC_PARAM`$ equivalent to our covariance-based version,
...
the $`r MDISC_PARAM`$ and $`r MIL_PARAM`$ parameters.
...
then compute the indices in the transformed space using
...
Strictly following @reckase_difficulty_1985's method in the original,
...
for dichotomous items different from the M2PL model.
