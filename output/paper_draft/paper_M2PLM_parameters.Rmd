...
<!-- # TODO: Use the "module" symbol instead of the "norm"? -->
...
The slope in direction $`r DIR_ANGLE_VEC`$ when $`r IRF_MAX_SLOPE_EQ`$ is given
by
...
so it does not apply in general to $`r DIR_COS_VEC`$.
Instead, we may consider the direction cosine vector with the standard basis
$`r LS_STD_BASIS`$.<!-- # TODO: Mario wrote "\mathcal{B}", I think he refers to "\mathcal{E}", the standard basis -->
Now, applying Proposition 2 (with $`r ALT_MATRIX_AS_ID`$) we obtain
...
We are thus considering the direction cosines with respect to the orthogonal
...
Substituting $`r DIR_COS_VEC_STD`$
...
<!-- # TODO: Verify that it's equation 20, not 18 -->
...
with basis $`r ITEM_SPACE_BASIS_EQ`$ and inner product matrix
$`r INNER_PROD_MATRIX_INV`$ can be defined,
...
\end{equation}
<!-- # TODO: Three equations collapsed into one altogether -->

It is clear that $`r INNER_PROD_INV_MAT_ELEMENT_EQ`$ were
$`r INNER_PROD_INV_MAT_ELEMENT`$ is the $`r INNER_PROD_INV_MAT_SUBINDEX`$-th
element of $`r INNER_PROD_MATRIX_INV`$.
<!-- # TODO: Error here? Mario means product of the basis elements? -->

Let $`r LS_STD_BASIS`$ be the standard basis of $`r TEST_SPACE`$.
<!-- # TODO: Can the standard basis of \Alpha be also \mathcal{E}? Otherwise, may I use \mathcal{E} instead? -->
Then, $`r BASIS_CHANGE_TS_EQ`$ is the change of basis matrix and
<!-- # TODO: Result pending of demonstration? -->
$`r DISCR_VECTOR_STD_MOD_EQ`$.

Applying propositon 1,
<!-- # TODO: Mario did not include item subindes, should I? -->
