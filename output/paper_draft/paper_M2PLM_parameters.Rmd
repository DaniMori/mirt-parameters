...
[M2PL, @mckinley_extension_1983] model is among the most important
...
a *multidimensional difficulty* parameter for M2PL items.
...
*multidimensional discrimination* (`r MDISC_PARAM`) parameter definition
for that model.
...
We can write all $`r TRAIT_VEC_IN_LS`$ as a linear combination of each basis,
$`r LS_BASIS`$ or $`r LS_STD_BASIS`$.
Therefore, we have both representations: $`r TRAIT_VECTOR_DEF_BASIS_EQ`$, and
$`r TRAIT_VECTOR_DEF_STD_BASIS_EQ`$. From now on we  will note  $`r TRAIT_COORDS_EQ`$ and
...
coordinates of $`r BASIS_VECTOR_ANY`$ in basis $`r LS_STD_BASIS`$.<!--# Does this need to be justified? -->
...
OBS. En la ecuación debe estar el producto inteno con el subindice que indique que es en la base B. No se ponen en las normas debido a la invarianza de estas. Como $\theta$ son las coordanadas en B se dejan igual.
...
OBS en esta primera ecuación recordar poner el subindice B en el producto interno
...
already assumes the orthonormality of the latent space.
...
AGREGADO
Aca pongo redacciön alternativa dejo igual la origunal debajo
...
We call $`r TEST_SPACE`$ the *test space* [@zhang_theoretical_1999] of the discrimination vectors.
...
Let us consider the model given by the equation (1 poner la referencia) where the latent traits and  the parameters $a_i$ are expressed in the same base B in their respective spaces. 

We have  $\theta^{'}=P \theta$ so since the probability is invariant, the parameters $a_i$ must also be transformed in $a^{'}_i$.

Equating the probabilities we get $a_1^{'}=P^{-1^T}a_i=M^{-1}Pa_i$ .
This shows that first a base change to the standard base is performed and then a transformation through $M^{-1}$, so the $a^{'}_i$ are expressed in the standard base.

Then, 

\begin{equation}
\|a_i^{'} \|=a_i^T P^{-1}P^{-1^T}a_i=a_i^TM^{-1}a_i
(\#eq:slope-param)
\end{equation}


Therefore, in the test space, a inner product matrix
$`r INNER_PROD_MATRIX_INV`$ can be defined,
such that $`r INNER_PROD_DISCR_EQ`$ to keep the norm invariant
