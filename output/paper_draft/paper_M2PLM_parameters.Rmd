...
Our aim is to find in the first place
...
with the sum and product by a scalar operations as in $`r REAL_N_SPACE`$.
Consider also an orthonormal basis $`r TEST_SPACE_STD_BASIS`$
in $`r TEST_SPACE`$, such that $`r DISCR_STD_COORDS_ALT_EQ`$.
Since the response probability to the `r MODEL_ACRONYM`,
given by \@ref(eq:M2PL-formulation), must be invariant with respect to
the change of basis expressed in \@ref(eq:change-basis),
we must define the coordinates $`r DISCR_STD_COORDS`$ of the parameter vector
$`r DISCR_VECTOR`$ in the orthonormal basis $`r TEST_SPACE_STD_BASIS`$,
...
Applying Proposition 1 to the model formulation in \@ref(eq:M2PL-formulation),
...
Applying Proposition 1, in the test space $`r TEST_SPACE`$, we get
