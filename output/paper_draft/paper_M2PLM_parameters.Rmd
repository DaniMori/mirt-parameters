...
<!-- # TODO: For all k? -->
If $`r BASIS_VECTOR_ANY_TRANSF_EQ`$, then

\begin{equation}
  `r TRANSFORM_MATRIX_EQ`.                           (\#eq:transform-matrix-def)
\end{equation}

With the standard basis in $`r LATENT_SPACE`$ we have the usual scalar product<!-- (i.e. dot product) -->.
With basis $`r LS_BASIS`$ in $`r LATENT_SPACE`$ we define an inner product
$`r LS_BASIS_INNER_PROD`$, such that the norm of $`r TRAIT_VECTOR`$ is
invariant, i.e., $`r TRAIT_NORM_EQ`$. Therefore,

\begin{equation}
  `r TRAIT_NORM_STD_SQ_EQ`.                             (\#eq:inner-product-def)
\end{equation}

Thus, we must define the inner product $`r LS_BASIS_INNER_PROD`$ with Gram
matrix $`r INNER_PROD_MAT_EQ`$. That is, $`r INNER_PROD_EQ`$, which keeps the
norm invariant.

The Gram matrix has $`r INNER_PROD_MAT_ELEMENT_EQ`$ as element. As
...
vectors of $`r LS_BASIS`$ and $`r LS_STD_BASIS`$, respectively.

::: {#prp-cosines-equivalence}
...
\begin{equation}
  `r DIAG_MATRIX_INNER_PROD_EQ`. <!-- # TODO: Continue here -->
\end{equation}
