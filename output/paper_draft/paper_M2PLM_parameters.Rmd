...
Before proceeding further, we need to make a clarification
...
The coordinates of $`r DISCR_VECTOR`$ are then $`r DISCR_COORDS`$.
Since the response probability must be invariant with respect to
a change of basis, $`r DISCR_COORDS`$ must also be transformed
into $`r DISCR_STD_COORDS`$, such that $`r INVARIANCE_EQ`$;
therefore, $`r DISCR_STD_COORDS_EQ`$.
This shows that first a transformation through $`r INNER_PROD_MATRIX_INV`$
is performed and then a change to the standard basis,
so the $`r DISCR_STD_COORDS`$ are expressed in the standard basis.
The norm of $`r DISCR_VECTOR`$ must then fulfill

\begin{equation}
  `r DISCR_VECTOR_MODULE_EQ`.                    (\#eq:discr-param-squared-norm)
\end{equation}

Therefore, an inner product $`r ITEM_SPACE_INNER_PROD`$ with Gram matrix
$`r INNER_PROD_MATRIX_INV`$ can be defined, such that $`r INNER_PROD_DISCR_EQ`$
keeps the norm of $`r DISCR_VECTOR`$ invariant.
The discrimination vectors are thus represented in their own space
$`r TEST_SPACE`$ with basis $`r ITEM_SPACE_BASIS`$,
which we refer to as the *test space* [@zhang_theoretical_1999].
The test space is in general different from the latent space $`r LATENT_SPACE`$,
