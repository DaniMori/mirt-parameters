...
To make @reckase_difficulty_1985's definition a particular case of ours
...
As $`r DISCR_VECTOR`$ is defined in space $`r TEST_SPACE`$ with basis
...
they are still two different spaces. <!-- # TODO: To discussion? -->
...
in the test space:
As both $`r LS_ORTH_BASIS`$ and $`r TEST_SPACE_STD_BASIS`$ are orthonormal,
we can map one directly onto the other. Without loss of generality, we may thus
apply Proposition 2, so if $`r ANGLE_VEC_TS_ITEM`$ is
...
<!-- # TODO: Revisar esto bien. Esto sólo aplica correctamente si efectivamente
"equiparamos" las bases U y U*. Además, en la Proposición 2 estamos asumiendo
que la "base de enlace" es la canónica, no una base ortonormal cualquiera. -->
...
On the basis of these conditions, we need to compute
...
$`r ORIGIN_ITEM_ORTH`$ and $`r END_ITEM_ORTH`$, respectively,
...
Given the mapping of $`r TEST_SPACE_STD_BASIS`$ onto $`r LS_ORTH_BASIS`$,
the direction cosines in $`r TEST_SPACE_STD_BASIS`$
are $`r DIR_COS_ITEM_VEC_STD`$.
Therefore,
...
Applying Equation \@ref(eq:dir-cos-vec-std),
for the origin coordinates we have that

\begin{equation}
  `r ORIGIN_ORTH_DISCR_EQ`                              (\#eq:origin-orth-discr)
\end{equation}

which, operating, results in

\begin{align}
  `r ORIGIN_ITEM_COORDS_EQ`    \\
  `r ORIGIN_TRANSF_SQUARED_EQ` \\
  `r ORIGIN_INNERPROD_EQ`      \\
  `r ORIGIN_INNERPROD_DIAG_EQ`
\end{align}

and, finally,

\begin{equation}
  `r ORIGIN_INNERPROD_DIR_COS_EQ`.                   (\#eq:origin-coords-result)
\end{equation}

The end coordinates can be computed by a similar procedure, which results in
...
Of course,
from Equations \@ref(eq:origin-orth-discr) and \@ref(eq:end-coords-result)
...
Therefore, for graphical representation purposes,
the correlation-based version will be preferred.
...
Therefore, we will need to define
a convenient value for $`r TRANSFORM_MATRIX_TRANSP_INV`$
...
However, it is worth noting that it has been defined ad-hoc
to fulfill this specific property.
Therefore it lacks other desirable properties of the covariance-based version,
like being invariant to a latent space scaling transformation,
or yielding a diagonal latent covariance matrix in the corresponding
orthogonal latent space basis.
...
by contrast, would require to compute the cosines in
...
The MIRT literature suggests in several instances that
"the angle between the axes [of latent ability space] represents
the degree of correlation" [@ackerman_multidimensional_2005-1, p. 16].
We hope our results help shed a clearer light on
this often missunderstood topic.
