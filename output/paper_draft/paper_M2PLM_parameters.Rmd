...
According to the M2PL model, the probability of a positive response to item
$`r ITEM_INDEX`$ is
...
where $`r DISCR_VECTOR`$ is a $`r N_DIMS`$-dimensional vector of discrimination
...
Let's consider the standard basis $`r LS_STD_BASIS_EQ`$ in $`r LATENT_SPACE`$.
TODO: Incomplete in Mario's version, is it correct to say "in the latent space"?
We can write all $`r TRAIT_VEC_IN_LS`$ as a linear combination of each baseis.
Therefore, we have both representations: $`r TRAIT_VECTOR_DEF_BASIS_EQ`$, and
$`r TRAIT_VECTOR_DEF_STD_BASIS_EQ`$, where $`r TRAIT_COORDS_EQ`$ and
$`r TRAIT_COORDS_STD_EQ`$.

Let $`r BASIS_CHANGE_DEF`$ be the change of basis matrix; then, we have that

\begin{equation}
  `r BASIS_CHANGE`.                                          (\#eq:change-basis)
\end{equation}

The $`r DIM_INDEX`$-th column of $`r TRANSFORM_MATRIX`$
...
read_chunk("src/Empirical_example_Tezza2018.R")
```
