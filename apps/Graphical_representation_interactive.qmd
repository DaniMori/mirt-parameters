...
This app show an interactive graphical representation of the Multidimensional parameters (location and discrimination) of the Multidimensional Compensatory Logistic IRT Model (MCLM).
...
# Render item in original coordinates:
...
output$ls_transformed <- renderPlot({
  ...
  lt_sample_transf() |> ggplot(aes(x = theta_1, y = theta_2)) +
    geom_vline(xintercept = 0, linewidth = LINE_WIDTH) +
    geom_abline( # Oblique `theta_1` axis
      intercept = 0,
      slope     = tan_vec_1,
      linewidth = LINE_WIDTH
    ) +
    geom_abline( # Oblique panel grid lines
      intercept = BREAKS * basis_vec_2[2],
      slope     = tan_vec_1,
      linewidth = LINE_WIDTH,
      linetype  = "C6"
    ) +
    ...
    scale_y_continuous(
      limits       = LIMITS,
      breaks       = BREAKS * basis_vec_2[2] + LIM_INF * basis_vec_1[2],
      labels       = BREAKS,
      name         = NULL,
      oob          = oob_keep
    ) +
    ...
    ggtitle("Transformed coordinates")
})

# Render item in transformed coordinates:
output$ts_transformed <- renderPlot({

  # Compute grid lines:
  
  ## Basis vectors:
  basis_vec_1 <- transf_inv_transp()[, 1]
  basis_vec_2 <- transf_inv_transp()[, 2]

  ## Oblique grid lines:
  tan_vec_2 <- basis_vec_2[2] / basis_vec_2[1] # Angle tangent
  ...
  output <- item_oblique() |>
    ...
    geom_hline(yintercept = 0L, linewidth = LINE_WIDTH)
  
  output <- if (basis_vec_2[1]) {
    ...
    output + 
      geom_vline(xintercept = 0L,     linewidth = LINE_WIDTH) +
      geom_vline(xintercept = BREAKS, linewidth = LINE_WIDTH, linetype  = "C6")
  }
  ...
  output +
    ...
    scale_x_continuous(
      limits       = LIMITS,
      breaks       = BREAKS * basis_vec_1[1] + LIM_INF * basis_vec_2[1],
      labels       = BREAKS,
      minor_breaks = NULL,
      name         = NULL,
      oob          = oob_keep
