...
that respondent with trait $`r LS_BASIS_EQ`$ gives a correct[^1] response is
given by
...
Let $`r BASIS_CHANGE_DEF`$ be the change of basis matrix.
...
[^2]: I'm preferring here the notation I use in the draft, that is,
    ...
    coordinates of $`r TRAIT_VECTOR`$ in $`r LS_BASIS`$,
    and $`r TRAIT_VEC_STD_EL`$ for the coordinates in $`r LS_STD_BASIS`$.
    ...
    ($`r TRAIT_VECTOR`$) for the "geometric vector" and
    the "algebraic vector" (i.e. column matrix) of its coordinates in
    $`r LATENT_SPACE`$ (this notation is usually accepted though).
