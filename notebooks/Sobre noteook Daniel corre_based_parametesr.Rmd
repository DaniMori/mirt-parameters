...
No es necesario hallar la matriz raiz cuadrada ya que su usamos 
$$K^T=Q (\Lambda)^{1/2}$$
también sirve ya que 
$$\mathbf{\Sigma}^{-1}=K^TK=Q (\Lambda)^{-1/2}   (\Lambda)^{-1/2} Q^{-1}$$
Por lo tanto 

$$K= (\Lambda)^{1/2} Q^T=(\Lambda)^{1/2} Q^{-1}$$
o sea

$$P= S (\Lambda)^{1/2} Q^{-1}$$

Podemos usar ls descomposicion de Cholesky



Como $\Sigma$ es simétrica y supongamos definida positiva entonces por Cholesky podemos escribir

$$\Sigma=L^T L$$
donde L es una matrz triangular superior, o sea $K =(L^{T})^{-1}$ de donde
...
```{r   echo=FALSE}
Sigma <- c(4,5, 4.8, 5, 25, 9, 4.8, 9, 9) |> matrix(ncol = 3)
S=diag(c(2,5,3))
R=cov_matrix <- c(1, .5, .8, .5, 1, .6, .8, .6, 1) |> matrix(ncol = 3)
...
#Calculo de P sin raiz cuadrada
P2<- S%*%lambda_root %*% Q_inv
#Calculo P cholesky
PCHOL<-S%*%solve(t(chol(Sigma)))

print("Matriz P por descomposición espectral ")
print(P)
print("Matriz P por método simplificado ")
print(P2)
print("Matriz P por Cholesky")
print(PCHOL)
```
...
```
Comprobacion usando P2
```{r   echo=FALSE}
print(round(P2 %*% Sigma %*% t(P2),3)) 
```
Comprobacion usando Cholesky
```{r   echo=FALSE}
print(round(PCHOL %*% Sigma %*% t(PCHOL)))
```
