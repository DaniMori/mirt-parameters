...
$(\mathbf{S} \mathbf{P}^{-1})^T = \mathbf{S} \mathbf{P}^{-1}$, y por lo tanto
$\mathbf{S} \mathbf{P}^{-1}$ es simétrica y (tal y como muestra ese mismo
resultado) $\mathbf{S}$ y $\mathbf{P}^{-1}$ son conmutativas.

Algo no parece encajar en esta deducción, ya que según eso todos los pares de
matrices simétricas (i.e. para cualesquiera $\mathbf{S}$ y $\mathbf{P}^{-1}$)
deberían dar lugar a una matriz simétrica y cumplir la propiedad de
conmutatividad.
