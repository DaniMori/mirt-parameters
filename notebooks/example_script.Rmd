...
La descomposicion espectral se expresa en la base canónica tanto para los datos
con coordenadas en la base $\mathfrak{B}$ como en la base $\mathfrak{E}$.
...
y lo propio con $\Sigma$ (con las coordenadas oblicuas)
...
$$
Q_E \Lambda_E Q_E^T = P Q \Lambda Q^T P^T
$$

Si tomamos la matriz $L$ definida como la matriz diagonal que contiene el
cociente de los valores propios de $\Lambda$ entre $\Lambda_E$

Entonces

$$
Q_E \Lambda_E Q_E^T = P Q L^{1/2} \Lambda_E (P Q L^{1/2} )^T
$$
...
Por otro lado la matriz de cambio de base entre $\mathfrak{B}$ y $\mathfrak{L}$
es $PQ_E^{-1}$ ya que $Q_E$ es la matriz de cambio de base entre $\mathfrak{B}$
y la base canónica

Luego la matriz de cambio de base queda

$$
PQ_E^{-1} = P( PQL^{1/2} )^{-1} = L^{-1/2} PQ^{-1} P^{-1}
$$

Se puede aplicar con esta matriz de cambio de base los resultados anteriores
hallados.
