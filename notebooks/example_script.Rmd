...
$$a_i^{'}=(P^{-1})^T a_i$$
se puede modelizar utilizando las coordenadas en la base canónica, es decir la ecuación anterior da la transformación de los a al transformar los $\theta$. Esto imlica que si utilizamos las coordeadas de las habilidades en la base can'onica debemos utilizar la $a'$.
...
Podemos presentar un gráfico de nibel mediante la función repnivel
...
El modelo de TRI expresa una relación en el espacio de coordenadas de la habilidad y la discriminación y en ese espacio de coordenadas $R^n$ se considera el producto punto.
En principio el rasgo $\theta$ tiene sus coordenadas expresadas en la base $\mathfrak{B}$ que representa a las reales dimensiones que tiene el rasgo. También las discriminaciones estan expresadas en alguna base que al no tener restricciones previas podemos suponer cualquier base que fijará las transformaciones del espacio.
En particular supondremos que la base de origen en el espacio $\mathcal A$ tiene como vectores las columnas de $(P^{-1})^T$. La llamaremos $\mathfrak{A}$
...
La matriz de cambio de base entre $\mathfrak{L}$ y $\mathfrak{E}$ es Q por lo tanto la matriz de cambio de base entre $\mathfrak{B}$ y $\mathfrak{L}$ es $$Q^{-1}P$$

Veamos un ejemplo emplendo como base ortogonal los vectores propios de $\Sigma$. El par'ametro a=(0.5,2) y d=1.1
